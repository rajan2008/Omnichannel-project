...
const categoryImages = {
  ...
  Storage: "https://images.unsplash.com/photo-1595514535415-eeabf1e6aa64?auto=format&fit=crop&q=80&w=300&h=300",
  Lighting: "https://images.unsplash.com/photo-1513506003901-1e6a229e2d15?auto=format&fit=crop&q=80&w=300&h=300",
  Decor: "https://images.unsplash.com/photo-1513694203232-719a280e022f?auto=format&fit=crop&q=80&w=300&h=300",
  Bedding: "https://images.unsplash.com/photo-1522771739844-6a9f6d5f14af?auto=format&fit=crop&q=80&w=300&h=300",
  Kitchen: "https://images.unsplash.com/photo-1556910103-1c02745aae4d?auto=format&fit=crop&q=80&w=300&h=300",
  Bathroom: "https://images.unsplash.com/photo-1584622650111-993a426fbf0a?auto=format&fit=crop&q=80&w=300&h=300",
  Furniture: "https://images.unsplash.com/photo-1505693314120-0d443867891c?auto=format&fit=crop&q=80&w=300&h=300",
  Appliances: "https://images.unsplash.com/photo-1581092160562-40aa08e78837?auto=format&fit=crop&q=80&w=300&h=300",
  Cleaning: "https://images.unsplash.com/photo-1585421514738-01798e348b17?auto=format&fit=crop&q=80&w=300&h=300",
...
const ProductCard = ({ product, onAddToCart, formatCurrency, onDeleteSuccess }) => {
  ...
  const canManage = user?.role === 'admin' || (user?.role === 'manager' && user?.store?._id === product.store);

  return (
    <div
      className={`group relative bg-white dark:bg-[#1a1c2c] rounded-2xl p-3 md:p-4 border border-slate-100 dark:border-white/5 transition-all hover:border-brand-red dark:hover:border-brand-red hover:shadow-2xl hover:shadow-brand-red/10 active:scale-[0.97] ${product.stock <= 0 ? "opacity-60 grayscale" : ""}`}
    >
      <div 
      ...
      <div className="space-y-0.5">
        <h3 className="font-bold text-xs md:text-sm text-slate-900 dark:text-white line-clamp-1">{product.name}</h3>
        <p className="text-[9px] md:text-[10px] text-slate-400 font-bold uppercase tracking-widest">{product.category}</p>
      </div>

      <div className="flex items-center justify-between mt-3">
        <span className="text-sm md:text-lg font-black text-brand-red">{formatCurrency(product.basePrice)}</span>
